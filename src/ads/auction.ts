...
export async function pickAd(opts: AuctionOpts): Promise<any> {
  ...
  const seen = new Set(recentClaims.map((c: any) => c.campaignId.toString()));
  const eligible = candidates.filter((c: any) => !seen.has(c._id.toString()));
  ...
  const totalBid = eligible.reduce((s: number, c: any) => s + Number(c.bidPerViewUsdc), 0);
  if (totalBid === 0) {
