...
import { AdImpression } from './models.js';
...
const lastHeartbeatTimes = new Map();
// Helpers
function generateImpressionToken(impressionId) {
...
const serveAdSlot = async (req, res) => {
    try {
        ...
        logger.error(`Error in /v1/ads/slot: ${error.message}`);
...
adsRouter.post('/v1/ads/heartbeat', async (req, res) => {
    try {
        ...
        logger.error(`Error in heartbeat endpoint: ${error.message}`);
        res.status(500).json({ error: error.message });
    }
});
...
adsRouter.post('/v1/ads/claim', async (req, res) => {
    ...
    if (body.impressionToken) {
        ...
        try {
            ...
            const updatedImpression = await Impression.findOneAndUpdate({ _id: impression._id, status: 'pending' }, { status: 'claimed', completedAt: new Date() }, { new: true });
            ...
            try {
                ...
                logger.error(`Failed to anchor batch during claim: ${anchorErr.message}`);
            }
            return res.json({
                creditJwt,
            ...
            if (error instanceof VerificationError) {
                // Log rejection
                try {
                    await Impression.findOneAndUpdate({ token: body.impressionToken, status: 'pending' }, { status: 'rejected', rejectionCode: error.code });
                }
                catch (e) {
                    logger.error(`Failed to reject impression: ${e.message}`);
                }
                return res.status(422).json({ error: error.message, code: error.code });
            }
            logger.error(`Claim verification error: ${error.message}`);
...
adsRouter.get('/v1/ads/click', async (req, res) => {
    try {
        ...
        logger.error(`Click attribution failed: ${error.message}`);
        res.status(400).send(`Invalid click token: ${error.message}`);
    }
});
// 19. GET /v1/ads/preview/:campaignId (marketer preview route)
adsRouter.get('/v1/ads/preview/:campaignId', requireMarketer, async (req, res) => {
    try {
...
adsRouter.post('/v1/privacy/delete-session', async (req, res) => {
...
adsRouter.post('/v1/users/auth/nonce', async (req, res) => {
...
adsRouter.post('/v1/users/auth/verify', async (req, res) => {
...
adsRouter.post("/v1/ads/start", requireUserAuth, async (req, res) => {
    const viewer = req.user.address.toLowerCase();
    const { surface, kind, modelInUse } = req.body;
    try {
