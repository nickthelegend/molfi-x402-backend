...
export async function x402Middleware(req, res, next) {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
        ...
        try {
            ...
            logger.warn(`Invalid or expired JWT token received: ${err.message}`);
        }
    }
    ...
    try {
        const decodedPayload = JSON.parse(Buffer.from(xPaymentHeader, 'base64').toString('utf-8'));
        if (decodedPayload.x402Version !== 1 ||
